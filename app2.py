...
def track_package():
    try:
        ...
        if trace_list:
            last_stage = trace_list[0]
            raw_status = last_stage.get('distance', 'Информация отсутствует')
            # Разделяем строку по "/" и берём часть после, убираем лишние пробелы
            parts = raw_status.split('/')
            russian_status = parts[1].strip() if len(parts) > 1 else raw_status
        else:
            russian_status = 'Информация отсутствует'

        stage_date = last_stage.get(
            'locateTime', 'Информация отсутствует') if trace_list else 'Информация отсутствует'
